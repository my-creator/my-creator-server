...
router.get('/:creatorIdx', async (req, res) => {
    ...
    const insertCreatorSearchQuery = `INSERT INTO creator_search(creator_idx) VALUES(${creatorIdx})`;
    const insertCreatorSearchResult = await db.queryParam_None(insertCreatorSearchQuery);

    const getCreatorProfileQuery = "SELECT * FROM creator WHERE idx = ?";
